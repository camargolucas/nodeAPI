...
exports.getLogin = function (req, res) {
  ...
  sql.execSqlQuery(query, res).then(ret => {
    if (ret == "") {
...
function verifyUserDevice(idUser, UUID, userDataResult, res) {
  ...
  let jsonRet;
  query =
    "SELECT UUID, idUsuario FROM PRE_LOGIN WHERE idUsuario = " + idUser + "";
  sql.execSqlQuery(query, res).then(ret => {
    ...
    if (ret == "") {
      ...
      query =
        "INSERT INTO PRE_LOGIN(UUID, idUsuario) VALUES ('" +
        UUID +
        "', " +
        idUser +
        ")";
...
exports.getSentStock = function (req, res) {
  ...
  let query =
    "SELECT COUNT(idEstoque) ENVIOS FROM PRE_ESTOQUE WHERE IDUSUARIO = " +
    idUsuario +
    " AND dataEstoque = '" +
    dateNow +
    "' ";
...
exports.getSentRequest = function (req, res) {
  ...
  let query =
    "SELECT COUNT(idPedido) ENVIOS FROM PRE_PEDIDO WHERE IDUSUARIO = " +
    idUsuario +
    " AND dataPedido = '" +
    dateNow +
    "' ";
